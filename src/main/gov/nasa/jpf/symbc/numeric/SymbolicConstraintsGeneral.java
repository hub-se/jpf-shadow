...
//			pb = new ProblemChoco2();
      ...
      pb = ProblemDReal.createInstance(new Config(new String[]{}));
		...
		if(result == null) {
			...
			//jpf-shadow: just added this option to suppress warnings
			if(ShadowInstructionFactory.debugConstraints) System.out.println("## Warning: timed out/ don't know (returned PC not-satisfiable) "+pc);
			return false;
		}
	...
	public boolean isSatisfiableGreen(final PathCondition pc) {
		...
		if(result == null) {
			...
			//jpf-shadow: added to optionally suppress solver warnings
			if(ShadowInstructionFactory.debugConstraints)  System.out.println("## Warning: timed out/ don't know (returned PC not-satisfiable) "+pc);
			return false;
		}
		if (result == Boolean.TRUE) {
