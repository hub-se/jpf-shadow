...
/*
 * Implementation of the shadow symbolic IFLT bytecode instruction
 */
			...
			if(!ti.isFirstStepInsn()){
				...
				case 2: //Concrete execution	
					if(ti.getExecutionMode() == Execute.BOTH){
						...
						old_pc._addDet(Comparator.LT, shadow_v, 0);
						BytecodeUtils.addConcreteValues(old_pc);

						new_pc._addDet(Comparator.LT, sym_v, 0);						
						...
						next_pc._addDet(Comparator.LT, expr_v, 0);
						BytecodeUtils.addConcreteValues(next_pc);

						if(next_pc.simplify()){
							pc._addDet(Comparator.LT, expr_v, 0);
							curCg.setCurrentPC(pc);
							return this.getTarget();
						}
						else{
							pc._addDet(Comparator.GE, expr_v, 0);
							curCg.setCurrentPC(pc);
							return this.getNext(ti);
						}
					}
