...
/*
 * Implementation of the shadow symbolic IF_ICMPEQ bytecode instruction
 */
...
public class IF_ICMPEQ extends gov.nasa.jpf.jvm.bytecode.IF_ICMPEQ{
	...
	public Instruction execute (ThreadInfo ti) {
		...
		if ((op_v1 == null) && (op_v2 == null)) { 
			return super.execute(ti); //Conditions are concrete
		}else{
			if(!ti.isFirstStepInsn()){
				...
				case 2: //Concrete execution	
					if(ti.getExecutionMode() == Execute.BOTH){
						...
						old_pc._addDet(Comparator.EQ, shadow_v1, shadow_v2);
						BytecodeUtils.addConcreteValues(old_pc);

						new_pc._addDet(Comparator.EQ, sym_v1, sym_v2);						
						...
						next_pc._addDet(Comparator.EQ, expr_v1, expr_v2);
						if(next_pc.simplify()){
