...
/*
 * Implementation of the shadow symbolic IFEQ bytecode instruction
 */
			...
			if(!ti.isFirstStepInsn()){
					...
					if(BytecodeUtils.isChangeBoolean(this, ti)){
						...
						if(prevCg.getExecutionMode() != Execute.BOTH ){
							if(prevCg.getNextChoice() == 2){
								...
								nextCg = new PCChoiceGenerator(3,4,1);
							}
						}
						else{
				...
				case 2: //Concrete execution	
					if(ti.getExecutionMode() == Execute.BOTH){
						...
						old_pc._addDet(Comparator.EQ, shadow_v, 0);
						BytecodeUtils.addConcreteValues(old_pc);

						new_pc._addDet(Comparator.EQ, sym_v, 0);						
						...
						next_pc._addDet(Comparator.EQ, expr_v, 0);
						
						if(next_pc.simplify()){
							pc._addDet(Comparator.EQ, expr_v, 0);
							curCg.setCurrentPC(pc);
							return this.getTarget();
						}
						else{
							pc._addDet(Comparator.NE, expr_v, 0);
							curCg.setCurrentPC(pc);
							return this.getNext(ti);
						}
					}
