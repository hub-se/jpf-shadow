...
/*
 * Implementation of the shadow symbolic IF_ICMPGE bytecode instruction
 */
			...
			if(!ti.isFirstStepInsn()){
				...
				case 2: //Concrete execution	
					if(ti.getExecutionMode() == Execute.BOTH){
						...
						old_pc._addDet(Comparator.GE, shadow_v1, shadow_v2);
						BytecodeUtils.addConcreteValues(old_pc);

						new_pc._addDet(Comparator.GE, sym_v1, sym_v2);						
						...
						next_pc._addDet(Comparator.GE, expr_v1, expr_v2);
						if(next_pc.simplify()){
							pc._addDet(Comparator.GE, expr_v1, expr_v2);
							curCg.setCurrentPC(pc);
							return this.getTarget();
						}
						else{
							pc._addDet(Comparator.LT, expr_v1, expr_v2);
							curCg.setCurrentPC(pc);
							return this.getNext(ti);
						}
					}
